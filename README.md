...
# Chạy benchmark với OpenAI API thật (khuyến nghị test nhỏ trước để tiết kiệm chi phí)
python run_benchmark.py --dataset data/hotpot_mini.json --out-dir outputs/openai_run --mode openai --min-questions 8
```

Lưu ý:
- `run_benchmark.py` có guard tự động để đảm bảo report đạt ngưỡng autograde tối thiểu 100 records (2 agent => tối thiểu 50 câu hỏi).
- Model OpenAI mặc định khuyến nghị là `gpt-4.1-nano` để dễ thấy khác biệt giữa ReAct và Reflexion.
