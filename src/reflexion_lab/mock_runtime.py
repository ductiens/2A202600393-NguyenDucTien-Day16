...
def _canonical_qid(qid: str) -> str:
    # Allow replicated datasets to suffix qid like "hp2_042".
    return qid.split("_", 1)[0]

def actor_answer(example: QAExample, attempt_id: int, agent_type: str, reflection_memory: list[str]) -> str:
    key = _canonical_qid(example.qid)
    if key not in FIRST_ATTEMPT_WRONG:
        return example.gold_answer
    if agent_type == "react":
        return FIRST_ATTEMPT_WRONG[key]
...
def reflector(example: QAExample, attempt_id: int, judge: JudgeResult) -> ReflectionEntry:
    strategy = "Do the second hop explicitly: birthplace city -> river through that city." if _canonical_qid(example.qid) == "hp2" else "Verify the final entity against the second paragraph before answering."
...
def failure_mode_for_qid(qid: str) -> str:
    return FAILURE_MODE_BY_QID.get(_canonical_qid(qid), "wrong_final_answer")
